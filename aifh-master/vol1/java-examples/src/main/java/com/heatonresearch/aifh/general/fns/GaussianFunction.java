...
public class GaussianFunction extends AbstractRBF {
    ...
    public double evaluate(final double[] x) {
        ...
        for (int i = 0; i < getDimensions(); i++) {
            ...
            double tmp01 = Math.pow(x[i] - center, 2);
            double tmp02 = 2.0 * width * width;
            double result = tmp01 / tmp02;
            value += Math.pow(x[i] - center, 2) / (2.0 * width * width);
        }
        // Math.exp(-value) : e^-6.699799596854386 = 0.0012311586060099874 (fex)
        double debugremovetmp = Math.exp(-value);  // jdcjdc todo remove
